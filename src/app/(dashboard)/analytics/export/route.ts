...
export async function GET(request: Request) {
  ...
  if (session.role !== "admin" && session.role !== "franchisee") return new Response("Forbidden.", { status: 403 });
  ...
  if (report === "weekly" || report === "monthly") {
    ...
    const rows = machineSalesReport(filtered, cadence, timeZone, range.from, range.to).map((row) => [
      row.period, row.periodStart, row.periodEnd, row.dataFrom, row.dataTo, row.partial ? "Yes" : "No", row.machine, row.machineId, row.imei, row.orders, row.units,
      row.gross.toFixed(2), row.refundedOrders, row.refunded.toFixed(2), row.net.toFixed(2),
      (row.orders ? row.net / row.orders : 0).toFixed(2),
    ]);
    return csvResponse([
      ["Period", "Period start", "Period end", "Sales included from", "Sales included to", "Partial period", "Machine", "Machine ID", "IMEI", "Net orders", "Net units", "Gross EUR", "Refunded orders", "Refunded EUR", "Net sales EUR", "Average order EUR"],
      ...rows,
