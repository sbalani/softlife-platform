...
export async function authorizedActionReport(s: SupabaseClient, actor: SessionProfile, reportId: string, requireDraft = false) {
  const { data, error } = await s.from("service_action_reports")
    .select("id,client_uuid,machine_id,operator_id,occurred_at,status,tenant_id,action_kind,action_modes,notes,cleaning_material_used,water_bucket_count,assigned_warehouse_id,source,revision")
