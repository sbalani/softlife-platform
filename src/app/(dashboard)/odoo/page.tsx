        ...
        {!production.available ? <p className="rounded-xl bg-warning/10 px-4 py-3 text-sm text-warning">Apply the Odoo manufacturing contract migration before configuring production.</p> : (
          <div className="space-y-6">
            ...
            <details id="production-conversion" open className="scroll-mt-4 rounded-xl border border-line">
              ...
              <div className="max-h-[28rem] overflow-auto border-t border-line"><table className="w-full min-w-[1120px] text-xs">
                ...
                <tbody className="divide-y divide-line">{production.products.map((product) => <tr key={product.id}>
                  ...
                  <td className="pr-3 font-semibold text-cocoa">{product.effective_quantity ?? "Missing"} {product.effective_uom ?? ""}<span className="block text-[10px] font-normal text-taupe">{product.effective_source}</span>{product.effective_source === "Legacy product default" && <span className="block text-[10px] font-normal text-warning">Saved on the ingredient record; it takes priority over the global default.</span>}{product.default_portion_size != null && product.effective_source !== "Legacy product default" && <span className="block text-[10px] font-normal text-warning">Legacy {product.default_portion_size} {product.default_portion_uom ?? "g"} remains underneath this override.</span>}</td>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <ProductionRunsPanel runs={production.runs} timeZone={tz} recipes={production.recipes} />
            </div>
          </div>
        )}
      </section>
