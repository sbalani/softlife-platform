...
export function FranchiseeIntakeForm({ locale }: { locale: Locale }) {
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div className="border-t border-[#e0d6cb] pt-5"><h3 className="font-bold text-[#4a3428]">{es ? "Datos opcionales" : "Optional business and payout details"}</h3><p className="mt-1 text-xs text-[#806f63]">{es ? "Puedes completarlos ahora o más tarde desde tu cuenta. Serán necesarios para poder realizar pagos." : "Complete these now or update them later from your account. They will be required before payments can be made."}</p></div>
