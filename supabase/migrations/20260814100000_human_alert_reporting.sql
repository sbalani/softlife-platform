...
WHERE alert.change_log_id = change.id
  ...
  AND change.field = 'material_empty';
...
WHERE alert.change_log_id = change.id
  ...
  AND change.metadata->>'raw_value' ~ '^\[(8|9|11|101|102|104|105|120|255)]';

UPDATE public.alerts alert
SET resolved_at = now()
FROM public.machine_status_snapshots snapshot
WHERE alert.machine_id = snapshot.machine_id
  AND alert.resolved_at IS NULL
  AND alert.change_log_id IN (SELECT id FROM public.machine_change_log WHERE field = 'device_online')
  AND snapshot.field = 'device_online'
  AND snapshot.value #>> '{}' = 'true';

WITH duplicates AS (
  SELECT alert.id, row_number() OVER (
    PARTITION BY alert.machine_id, alert.change_alert_rule_id, COALESCE(alert.product_id::TEXT, ''), COALESCE(change_field.field, '')
    ORDER BY alert.created_at DESC
  ) AS rank
  FROM public.alerts alert
  LEFT JOIN public.machine_change_log change_field ON change_field.id = alert.change_log_id
  WHERE alert.resolved_at IS NULL
    AND alert.change_alert_rule_id IS NOT NULL
)
UPDATE public.alerts alert
SET resolved_at = now()
FROM duplicates
WHERE alert.id = duplicates.id AND duplicates.rank > 1;
...
BEGIN
  ...
  IF NEW.machine_id IS NOT NULL THEN
    SELECT tenant_id, COALESCE(display_name, name, device_imei)
    INTO machine_tenant, machine_label
    ...
    IF matches THEN
      ...
      IF NOT FOUND THEN
        INSERT INTO public.alerts (
          ...
          title, message, change_log_id, change_alert_rule_id
        ) VALUES (
          machine_tenant, NEW.field, rule.severity, NEW.machine_id, NEW.product_id,
          NEW.entity_key, alert_title, alert_message, NEW.id, rule.id
        );
      END IF;
    ELSE
...
BEGIN
  ...
  IF counts IS NOT NULL THEN
    remaining_cups := counts[1]::INTEGER;
    total_cups := counts[2]::INTEGER;
  END IF;
  ...
  IF current_pct <= 25 THEN
    ...
    WHERE alert.type = 'material_remaining_critical'
    ...
    IF NOT FOUND THEN
      INSERT INTO public.alerts (
        ...
        'material_remaining_pct', alert_title, alert_message, current_pct, NEW.id
      );
    END IF;
  ELSE
...
SET title = CASE change.field
      ...
      WHEN 'ordering_system_fault' THEN 'Unrecognized machine fault'
      WHEN 'stock' THEN 'Product unavailable'
      ELSE COALESCE(alert.title, 'Machine needs attention')
    END,
    message = CASE change.field
      WHEN 'cup_empty' THEN 'The machine reports no cups and may be unable to sell. Refill the cup dispenser.'
      WHEN 'device_online' THEN 'No Huaxin connection was reported in two consecutive checks. Current sales status is unknown.'
      WHEN 'cup_foreign_object' THEN 'A foreign object was detected in the cup holder. Inspect and clear the cup area.'
      WHEN 'cup_blocked' THEN 'The cup dispenser reports a blockage. Inspect the dispenser before the next sale.'
      WHEN 'cup_take_fault' THEN 'A prepared cup may still be waiting for collection. Check the dispensing area.'
      WHEN 'mixture_ratio_fault' THEN 'The machine cannot maintain the configured mixture proportion. Inspect the product supply.'
      WHEN 'ordering_system_fault' THEN 'Huaxin reported ' || COALESCE(change.metadata->>'raw_value', 'an unknown operating state') || '. Inspect the machine.'
      WHEN 'stock' THEN COALESCE(product.name, change.entity_key, 'A product') || ' stock counter reached zero. The product is unavailable for sale.'
      ELSE alert.message
    END
FROM public.machine_change_log change
LEFT JOIN public.products product ON product.id = change.product_id
WHERE alert.change_log_id = change.id;

UPDATE public.alerts
SET title = CASE WHEN remaining_pct = 0 THEN 'Material exhausted' ELSE 'Material critically low' END,
    message = CASE WHEN remaining_pct = 0
      THEN 'No post-shortage sales remain. The machine should have stopped selling.'
      ELSE remaining_pct || '% of post-shortage sales remain. Refill soon.' END
WHERE type = 'material_remaining_critical';

UPDATE public.alerts SET title = COALESCE(title, 'Machine needs attention');
ALTER TABLE public.alerts ALTER COLUMN title SET NOT NULL;
...
WITH cup_states AS (
  SELECT machine.id AS machine_id,
         COALESCE(cup.raw, operating.raw) AS raw,
         COALESCE(cup.observed_at, operating.observed_at) AS observed_at
  FROM public.machines machine
  LEFT JOIN public.machine_status_snapshots cup
    ON cup.machine_id = machine.id AND cup.field = 'raw:status_0_cuplack'
  LEFT JOIN public.machine_status_snapshots operating
    ON operating.machine_id = machine.id AND operating.field = 'raw:status_0_os'
  WHERE lower(COALESCE(cup.raw->>'value', '')) IN ('1','true','abnormal','anomaly','anomalies','anomalía','anomalías','anomalia','anomalias')
     OR COALESCE(cup.raw->>'data', '') = '1'
     OR COALESCE(operating.raw->>'value', '') ~ '^\[101]'
)
INSERT INTO public.machine_status_snapshots (machine_id, field, value, raw, observed_at)
SELECT machine_id, 'cup_empty', 'true'::JSONB, raw, COALESCE(observed_at, now()) FROM cup_states
ON CONFLICT (machine_id, field) DO UPDATE
SET value = EXCLUDED.value, raw = EXCLUDED.raw, observed_at = EXCLUDED.observed_at;
...
SELECT machine.id, machine.device_imei, COALESCE(machine.display_name, machine.name),
       'machine_sync', 'status_changed', 'machine_status', 'cup_empty', 'cup_empty',
       'true'::JSONB, jsonb_build_object(
         'description', cup.raw->>'desc', 'raw_value', cup.raw->>'value', 'raw_data', cup.raw->>'data'
       )
FROM public.machines machine
JOIN public.machine_status_snapshots cup ON cup.machine_id = machine.id AND cup.field = 'cup_empty'
WHERE cup.value #>> '{}' = 'true'
  AND NOT EXISTS (
    SELECT 1 FROM public.alerts alert
    JOIN public.machine_change_log change ON change.id = alert.change_log_id
    WHERE alert.machine_id = machine.id AND alert.resolved_at IS NULL AND change.field = 'cup_empty'
  );

WITH material_out_states AS (
  SELECT machine.id AS machine_id, operating.raw, operating.observed_at
  FROM public.machines machine
  JOIN public.machine_status_snapshots operating
    ON operating.machine_id = machine.id AND operating.field = 'raw:status_0_os'
  WHERE COALESCE(operating.raw->>'value', '') ~ '^\[102]'
)
INSERT INTO public.machine_status_snapshots (machine_id, field, value, raw, observed_at)
SELECT machine_id, 'material_out', 'true'::JSONB, raw, observed_at FROM material_out_states
ON CONFLICT (machine_id, field) DO UPDATE
SET value = EXCLUDED.value, raw = EXCLUDED.raw, observed_at = EXCLUDED.observed_at;
