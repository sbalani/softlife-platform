...
export function canAccessPublicIncident(role: IncidentViewer["role"]): boolean {
...
export function incidentAccessFilter(viewer: IncidentViewer): string | null {
  ...
  if (viewer.role === "operator") return `source_kind.eq.public,assigned_user_id.eq.${viewer.id},created_by.eq.${viewer.id}`;
  if (!viewer.tenant_id) return "id.eq.00000000-0000-0000-0000-000000000000";
  return `and(source_kind.neq.public,or(owning_tenant_id.eq.${viewer.tenant_id},assigned_tenant_id.eq.${viewer.tenant_id},assigned_user_id.eq.${viewer.id},created_by.eq.${viewer.id}))`;
}
