...
export function ActionReportStockRetry({ reportId }: { reportId: string }) {
  ...
  function retry() {
    ...
    startTransition(async () => {
      ...
      setMessage(body.status === "needs_review" ? "Captured; some product mappings need review." : "Menu stock observation captured.");
      router.refresh();
    });
  }

  return <div className="mt-3 flex flex-wrap items-center gap-2 rounded-lg bg-warning/10 px-3 py-2">
    <span className="text-xs font-semibold text-warning">This confirmed refill has no menu-stock observation.</span>
