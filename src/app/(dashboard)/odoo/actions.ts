...
export async function saveProductionProduct(_state: OdooActionResult | null, fd: FormData): Promise<OdooActionResult> {
  try {
    const s = await productionAdminClient();
    ...
    if (useGlobal) {
      if (!consumptionType) throw new Error("Classify the ingredient before using a global default.");
      const { data: globalDefault, error: defaultError } = await s.from("production_consumption_defaults")
        .select("consumption_type").eq("consumption_type", consumptionType).maybeSingle();
...
export async function saveProductionSettings(_state: OdooActionResult | null, fd: FormData): Promise<OdooActionResult> {
  try {
    const s = await productionAdminClient();
