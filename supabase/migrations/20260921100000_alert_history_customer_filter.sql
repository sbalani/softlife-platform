...
SELECT a.id, a.type, a.severity, a.title, a.message, a.remaining_pct, a.created_at,
       ...
       customer.tenant_name AS customer_name
...
LEFT JOIN LATERAL (
  SELECT assignment.tenant_id, tenant.name AS tenant_name
  FROM public.machine_franchisee_assignments assignment
  JOIN public.tenants tenant ON tenant.id = assignment.tenant_id
  WHERE assignment.machine_id = a.machine_id
    AND assignment.start_date <= (a.created_at AT TIME ZONE 'Europe/Madrid')::DATE
    AND (assignment.end_date IS NULL OR assignment.end_date >= (a.created_at AT TIME ZONE 'Europe/Madrid')::DATE)
