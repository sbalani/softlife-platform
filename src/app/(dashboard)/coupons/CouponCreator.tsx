  ...
  return (
    <form action={action} className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <label className="sm:col-span-2"><span className={label}>Location label</span><input name="localName" value={values.localName} onChange={(event) => set("localName", event.target.value)} placeholder="Málaga" className={`w-full ${input}`} /></label>
      </div>
      ...
      <fieldset className="rounded-xl border border-line p-3">
        ...
        <p className="mb-2 text-xs text-taupe">No selection applies the coupon to all machines.</p>
      ...
      <div className="rounded-xl border border-line bg-cream/40 p-3">
        ...
        {values.couponType === "2" && <label><span className={label}>Uses per card</span><input name="secondary" type="number" min="1" required value={values.secondary} onChange={(event) => set("secondary", event.target.value)} className={`w-24 ${input}`} /></label>}
      </div>
