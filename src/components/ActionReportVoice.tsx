...
export function ActionReportVoice({ reportId, notesLength, onPendingChange, onTranscript }: { reportId: string | null; notesLength: number; onPendingChange: (pending: boolean) => void; onTranscript: (transcript: string) => void }) {
  ...
  async function upload(blob: Blob) {
    if (!reportId) { setStaged(blob); return; }
    setUploading(true);
    ...
    try {
      ...
      const completeResponse = await fetch(`/api/action-reports/${reportId}/audio-upload/complete`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: signed.path, mime_type: signed.mime_type, purpose: "notes" }) });
      ...
      setMessage("Audio queued for notes transcription.");
      setStaged(null);
  ...
  const attachAfterDraftSave = useEffectEvent(() => {
    if (reportId && staged && !uploading) void upload(staged);
  });
  useEffect(() => {
  ...
  async function startRecording() {
    ...
    if (!mimeType || !navigator.mediaDevices?.getUserMedia) { onPendingChange(false); setMessage("Audio recording is not supported in this browser."); return; }
    try {
  ...
  return <div className="mt-2">
    <div className="flex flex-wrap items-center gap-2">
      {!staged && !status?.job && <button type="button" onClick={recording ? stopRecording : startRecording} disabled={uploading} aria-label={recording ? "Stop recording notes" : "Record notes"} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-bold text-white disabled:opacity-50 ${recording ? "bg-danger" : "bg-cocoa"}`}><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="2" width="6" height="12" rx="3" /><path d="M5 10a7 7 0 0 0 14 0M12 17v5M8 22h8" /></svg>{recording ? "Stop recording" : "Record notes"}</button>}
      {recording && <span className="text-sm font-semibold text-danger">Recording notes, maximum 3 minutes...</span>}
      {staged && reportId && <button type="button" onClick={() => { void upload(staged); }} disabled={uploading} className="rounded-lg bg-terracotta px-3 py-2 text-sm font-bold text-white disabled:opacity-50">{uploading ? "Attaching..." : "Attach recording"}</button>}
    ...
    <p className="mt-2 text-xs text-taupe">Record notes before saving. Unsaved audio stays only in this browser tab and is discarded if you leave.</p>
