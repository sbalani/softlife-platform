...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!report || (config.kind === "audio" && report.status !== "draft")) return Response.json({ error: { message: "Report not found or immutable" } }, { status: 404 });
    if (config.kind === "audio") {
      const { data: existing } = await s.from("service_action_ai_jobs").select("id").eq("report_id", reportId).maybeSingle();
