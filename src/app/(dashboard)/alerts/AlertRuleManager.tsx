...
import { deleteAlertRule, saveAlertRule, setAlertRuleEnabled, setAlertRuleMobileNotification, type AlertRuleResult } from "./actions";
...
const STATUS_OPTIONS: Record<string, [string, string][]> = {
  ...
  mixture_ratio_fault: [["true", "Insufficient proportion"], ["false", "Normal"]],
};

function conditionLabel(rule: ChangeAlertRule) {
  if (rule.rule_type !== "status_equals") return `${rule.min_value ?? "-∞"} to ${rule.max_value ?? "∞"}${rule.field === "temperature" ? "°C" : ""}`;
  ...
  return (
    <section className="mb-6 rounded-2xl border border-line bg-white p-5">
      ...
      <form action={action} className="flex flex-wrap items-end gap-3">
        ...
        {field === "temperature" && <label><span className={label}>Temperature series</span><input name="series_name" placeholder="e.g. pre-cooling" className={`w-40 ${input}`} /><span className="mt-1 block text-[10px] text-taupe">Blank applies to every series.</span></label>}
        ...
        <label className="flex items-center gap-2 pb-2 text-xs font-semibold text-cocoa"><input type="checkbox" name="notify_mobile" className="accent-terracotta" />Mobile notification</label>
      ...
      {rules.length > 0 && <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[940px] text-sm"><thead className="text-left text-[11px] uppercase text-taupe"><tr><th className="py-2">Rule</th><th>Field</th><th>Series</th><th>Product scope</th><th>Machine scope</th><th>Condition</th><th>Severity</th><th>Mobile</th><th /></tr></thead><tbody className="divide-y divide-line">{rules.map((rule) => <tr key={rule.id} className={rule.enabled ? "" : "opacity-50"}><td className="py-2 font-semibold text-cocoa">{rule.name}</td><td>{rule.field.replaceAll("_", " ")}</td><td>{rule.series_name ?? "All"}</td><td>{rule.product_name ?? "All products"}</td><td>{rule.machine_name ?? "All machines"}</td><td>{conditionLabel(rule)}</td><td className="capitalize">{rule.severity}</td><td><button disabled={updating} onClick={() => startTransition(() => setAlertRuleMobileNotification(rule.id, !rule.notify_mobile))} className={`rounded-full px-2 py-1 text-[10px] font-bold ${rule.notify_mobile ? "bg-danger/10 text-danger" : "bg-cream text-taupe"}`}>{rule.notify_mobile ? "Push on" : "In-app"}</button></td><td className="space-x-3 text-right"><button disabled={updating} onClick={() => startTransition(() => setAlertRuleEnabled(rule.id, !rule.enabled))} className="font-semibold text-terracotta">{rule.enabled ? "Pause" : "Enable"}</button><button disabled={updating} onClick={() => confirm("Delete this alert rule?") && startTransition(() => deleteAlertRule(rule.id))} className="font-semibold text-danger">Delete</button></td></tr>)}</tbody></table></div>}
    </section>
  );
}
