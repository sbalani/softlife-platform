CREATE TABLE IF NOT EXISTS public.odoo_lot_stock (
  odoo_lot_id INTEGER NOT NULL REFERENCES public.odoo_lots(odoo_id) ON DELETE CASCADE,
  ...
  PRIMARY KEY (odoo_lot_id, odoo_warehouse_id)
);
CREATE INDEX IF NOT EXISTS odoo_lot_stock_warehouse_idx ON public.odoo_lot_stock(odoo_warehouse_id);
ALTER TABLE public.odoo_lot_stock ENABLE ROW LEVEL SECURITY;
CREATE POLICY odoo_lot_stock_read ON public.odoo_lot_stock FOR SELECT USING (true);
  ...
  IF v_has_refill THEN
    ...
    IF v_refill_inserted THEN
      FOR v_line IN SELECT value FROM jsonb_array_elements(p_refill_lines) LOOP
        ...
        SELECT l.odoo_id, l.name, l.odoo_product_id, l.product_name,
          COALESCE(s.qty, CASE WHEN l.odoo_warehouse_id = v_machine.odoo_warehouse_id THEN l.qty END) AS qty
        INTO v_lot
        FROM public.odoo_lots l
        LEFT JOIN public.odoo_lot_stock s ON s.odoo_lot_id = l.odoo_id AND s.odoo_warehouse_id = v_machine.odoo_warehouse_id
        WHERE l.odoo_id = (v_line->>'odoo_lot_id')::INTEGER
        FOR UPDATE OF l;
        IF NOT FOUND THEN RAISE EXCEPTION 'Lot not found'; END IF;
        IF v_lot.qty IS NULL THEN RAISE EXCEPTION 'Lot is not in this machine warehouse'; END IF;
        ...
        WHERE lu.odoo_lot_id = v_lot.odoo_id
          AND r.odoo_sync_status IN ('pending', 'failed')
          AND r.payload_json->>'odoo_warehouse_id' = v_machine.odoo_warehouse_id::TEXT;
