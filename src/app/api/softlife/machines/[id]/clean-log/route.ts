...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const bucketRaw = body.water_bucket_count;
  ...
  try {
    ...
    if (materialUsed === null || (waterBuckets !== null && (!Number.isInteger(waterBuckets) || waterBuckets < 0 || waterBuckets > 20))) return Response.json({ error: { message: "Invalid cleaning evidence" } }, { status: 400 });
