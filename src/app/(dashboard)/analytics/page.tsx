...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<AnalyticsParams> }) {
  ...
  return (
    <div>
      ...
      <section className="mt-6 rounded-2xl border border-line bg-white p-5"><h2 className="mb-1 font-display text-lg font-bold text-cocoa">Topping consumption</h2><p className="mb-3 text-xs text-taupe">Servings selected across standalone and combination orders, multiplied by order quantity. Bases and sauces are excluded using the product catalog.</p>{toppingBars.length ? <HBarChart data={toppingBars} color="#d47e54" unit="×" /> : <p className="py-6 text-center text-sm text-taupe">No topping consumption matches these filters.</p>}<div className="mt-4 overflow-x-auto"><table className="w-full min-w-[480px] text-sm"><thead className="text-left text-[11px] uppercase text-taupe"><tr><th className="py-2">Topping</th><th className="text-right">Orders</th><th className="text-right">Servings</th></tr></thead><tbody className="divide-y divide-line">{toppingRows.map((row) => <tr key={row.name}><td className="py-2 font-semibold text-cocoa">{row.name}</td><td className="text-right">{row.orders}</td><td className="text-right font-bold">{row.servings}</td></tr>)}</tbody></table></div></section>
