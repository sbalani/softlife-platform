...
function PublicReportDetails({ incident }: { incident: Incident }) {
  if (incident.sourceKind !== "public" || !incident.publicReport) return null;
  const report = incident.publicReport;
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {active.map((incident) => {
            ...
            return <article id={`incident-${incident.id}`} key={incident.id} className={`rounded-2xl border p-5 ${SEVERITY[incident.severity]}`}>
              ...
              <PublicReportDetails incident={incident} />
