...
export async function GET(request: Request) {
  ...
  const records = ((data as Record<string, unknown>[]) ?? []).map((row) => {
    ...
    const payload = row.mobile_draft_payload && typeof row.mobile_draft_payload === "object" ? row.mobile_draft_payload as Record<string, unknown> : {
      ...
      refill_lines: serverLines.map((line) => ({ server_line_id: line.id, quantity: line.quantity, unit: line.unit, product_name: line.product_name, lot_code: line.observed_lot_code, odoo_lot_id: line.observed_odoo_lot_id, finished_bottle: line.finished_bottle, left_unfinished_bottle: line.left_unfinished_bottle, inventory_quantity: line.inventory_quantity, provenance_status: line.provenance_status, unresolved_reason: line.unresolved_reason })),
    };
