...
import { getDailyWeather } from "@/lib/data/weather";
...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<AnalyticsParams> }) {
  ...
  const weatherMachines = machines.filter((machine) => (!selectedMachineId || machine.id === selectedMachineId) && (currentMachineIds === null || currentMachineIds.has(machine.id))).map((machine) => ({
    ...machine,
    activeRanges: machineAccess === null ? undefined : machineAccess.filter((period) => period.machine_id === machine.id).map((period) => ({ from: [period.start_date, range.from].sort().at(-1)!, to: [period.end_date ?? range.to, range.to].sort()[0] })),
  }));
  ...
  const weatherTrend = selectedWeather === "off" ? undefined : weatherResult.weather.map((weather) => ({
    day: weather.day,
    label: new Date(`${weather.day}T12:00:00Z`).toLocaleDateString("en", { day: "numeric", month: "short", timeZone: "UTC" }),
    value: selectedWeather === "rain" ? weather.precipitation : weather.temperatureMean,
    minimum: weather.temperatureMin,
    maximum: weather.temperatureMax,
  ...
  return (
    <div>
      ...
      <section className="mt-6 rounded-2xl border border-line bg-white p-5"><h2 className="mb-1 font-display text-lg font-bold text-cocoa">Topping consumption</h2><p className="mb-3 text-xs text-taupe">Solid and liquid topping servings selected across standalone and combination orders, multiplied by order quantity. The base lane is excluded, with the product catalog used as an additional check.</p>{toppingBars.length ? <HBarChart data={toppingBars} color="#d47e54" unit="×" /> : <p className="py-6 text-center text-sm text-taupe">No topping consumption matches these filters.</p>}<div className="mt-4 overflow-x-auto"><table className="w-full min-w-[480px] text-sm"><thead className="text-left text-[11px] uppercase text-taupe"><tr><th className="py-2">Topping</th><th className="text-right">Orders</th><th className="text-right">Servings</th></tr></thead><tbody className="divide-y divide-line">{toppingRows.map((row) => <tr key={row.name}><td className="py-2 font-semibold text-cocoa">{row.name}</td><td className="text-right">{row.orders}</td><td className="text-right font-bold">{row.servings}</td></tr>)}</tbody></table></div></section>
